...
const regexCls = /[.]+cls/gm;
const regexCls2 = /class="+cls/gm;
...
const changeClasses = async () => {
  ...
  const newSVGdata = str.replace(regex && regexCls, m => m[0] + fileName + m.substr(1))
    .replace(regex2 && regexCls2, m => m.substr(0, 7) + fileName + m.substr(7));
